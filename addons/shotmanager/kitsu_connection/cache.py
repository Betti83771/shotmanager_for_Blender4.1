...
_scene_enum_list: List[Tuple[str, str, str]] = []
...
def project_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active project to %s", _project_active.name)
...
def episode_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active episode to %s", _episode_active.name)
...
def sequence_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active sequence to %s", _sequence_active.name)
...
def shot_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active shot to %s", _shot_active.name)
...
def asset_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active asset to %s", _asset_active.name)
...
def asset_type_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active asset type to %s", _asset_type_active.name)
...
def task_type_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active task type to %s", _task_type_active.name)
...
def edit_active_set_by_id(context: bpy.types.Context, entity_id: str) -> None:
    ...
    print("Set active edit to %s", _task_type_active.name)
...
def get_scenes_enum_for_active_seq(
    self: bpy.types.Operator, context: bpy.types.Context
) -> List[Tuple[str, str, str]]:
    global _scene_enum_list
    global _scene_cache_seq_id
    ...
    if _scene_cache_seq_id == seq_active.id:
        return _scene_enum_list

    # Update Cache Sequence ID
    _scene_cache_seq_id = seq_active.id

    _scene_enum_list.clear()
    _scene_enum_list.extend(
        [(s.id, s.name, s.description or "") for s in seq_active.get_all_scenes()]
    )
    return _scene_enum_list
...
def load_user_all_tasks(context: bpy.types.Context) -> List[Task]:
    ...
    print("Loaded assigned tasks for: %s", _user_active.full_name)
...
def init_startup_variables(context: bpy.types.Context) -> None:
    ...
    if not addon_prefs.session.is_auth():
        print("Skip initiating startup cache. Session not authorized")
        return

    if _cache_startup_initialized:
        print("Startup Cache already initiated")
        return

    # User.
    _user_active = User()
    print("Initiated active user cache to: %s", _user_active.full_name)

    # User Tasks.
    load_user_all_tasks(context)
    print("Initiated active user tasks")
...
def clear_startup_variables():
    ...
    _user_active = User()
    print("Cleared active user cache")

    _user_all_tasks.clear()
    _update_tasks_collection_prop(bpy.context)
    print("Cleared active user all tasks cache")
...
def init_cache_variables() -> None:
    ...
    if not addon_prefs.session.is_auth():
        print("Skip initiating cache. Session not authorized")
        return

    if _cache_initialized:
        print("Cache already initiated")
        return
...
def clear_cache_variables():
    ...
    _shot_active = Shot()
    print("Cleared active shot cache")

    _asset_active = Asset()
    print("Cleared active asset cache")

    _sequence_active = Sequence()
    print("Cleared active aequence cache")

    _asset_type_active = AssetType()
    print("Cleared active asset type cache")

    _project_active = Project()
    print("Cleared active project cache")

    _task_type_active = TaskType()
    print("Cleared active task type cache")
