...
class KITSU_addon_preferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        if not self.session.is_auth():
            ...
            box.row().operator("sm_kitsu.session_start", text="Login", icon="PLAY")
        else:
            row = box.row()
            row.prop(self, "host")
            row.enabled = False
            box.row().label(text=f"Logged in: {self.session.email}")
            box.row().operator("sm_kitsu.session_end", text="Logout", icon="PANEL_CLOSE")
...
def project_root_dir_get(context: bpy.types.Context):
    addon_prefs = addon_prefs_get(context)
    path = Path(addon_prefs.project_root_dir)
    return path
