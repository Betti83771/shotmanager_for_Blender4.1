...
from ..context.ops import KITSU_OT_con_detect_context, KITSU_OT_con_set_asset, KITSU_OT_con_set_asset_th
...
class KITSU_PT_vi3d_context(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        if self.poll_error(context):
            box = draw_error_box(layout)
            if not project_active:
                draw_error_active_project_unset(box)
...
class KITSU_PT_set_asset(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.operator(KITSU_OT_con_set_asset_th.bl_idname)
