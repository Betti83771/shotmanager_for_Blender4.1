...
from ..types import TaskType, AssetType, Task, TaskStatus
...
class KITSU_OT_con_productions_load(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        context.window_manager.choose_prod_popup_running = True
        context.window_manager.invoke_search_popup(self)
        return {"FINISHED"}
    
    def cancel(self, context):
        context.window_manager.choose_prod_popup_running = False
        return super().cancel()
...
class KITSU_OT_con_set_asset(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        if not bpy.data.filepath.startswith(str(prefs.project_root_dir_get(context))):
            print(bpy.data.filepath, str(prefs.project_root_dir_get(context)))
...
class KITSU_OT_con_set_asset_th(bpy.types.Operator):
    bl_idname = "kitsu.con_set_asset_th"
    bl_label = "Set Kitsu Asset Thumbnail"
    bl_description = ""
    ...
    def execute(self, context):
        ...
        if not kitsu_asset:
            self.report({"ERROR"}, "Failed to find active Kitsu Asset")
            return {"CANCELLED"}
        task_type_entity = TaskType.by_name("Modelling")
        task = Task.by_name(kitsu_asset, task_type_entity)
        task_status = TaskStatus.by_short_name("WIP")
        print("task_status", task_status)
        comment = task.add_comment(
            task_status,
            comment="test thumbnail",
        )
        ...
        self.report(
            {"INFO"},
            f"Kitsu Asset {kitsu_asset.name} preview added",
        )
        return {"FINISHED"}
