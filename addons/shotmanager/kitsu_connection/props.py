    ...
    if len(matches) == 0:
        ...
        for to_replace in matches:
            if to_replace in var_lookup_table:
                ...
                print("Failed to resolve variable: %s not defined!", to_replace)
                return ""
        return result
...
class KITSU_property_group_scene(bpy.types.PropertyGroup):
    ...
    # Scene
    ###########
    scene_active_id: bpy.props.StringProperty(  # type: ignore
        name="Active Scene ID",
        description="ID that refers to the active scene on server",
        default="",
    )


    scene_active_name: bpy.props.StringProperty(
        name="Scene",
        description="Scene",
        default="",  # type: ignore
        #options=set(),
...
def _add_window_manager_props():
    bpy.types.WindowManager.choose_prod_popup_running = bpy.props.BoolProperty(
        default=False,
    )
...
def update_sequence_colors_coll_prop(dummy: Any) -> None:
    ...
    for seq in sequences:
        ...
        if seq.kitsu.sequence_id not in sequence_colors.keys():
            print("Added %s to scene.kitsu.seqeuence_colors", seq.kitsu.sequence_name)
