...
class FolderListModel:
    ...
    def root_path(self, path: Path) -> None:
        if not path or not path.absolute().exists():
            print("FolderListModel: Path does not exist: %s", str(path))
            self.reset()
        else:
            self.__root_path = path
            print("FolderListModel: Root path  was set to %s", path.as_posix())
...
class FileListModel:
    ...
    def root_path(self, path: Path) -> None:
        if not path or not path.absolute().exists():
            print("FileListModel: Path does not exist: %s", str(path))
            self.reset()
        else:
            self.__root_path = path
            self.__load_dir(self.__root_path)
