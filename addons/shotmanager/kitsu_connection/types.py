...
def project_root_dir_get(context: bpy.types.Context):
    ...
    path = Path(addon_prefs.project_root_dir).joinpath('svn')
   # print(path)
    return path
...
class Session:
    ...
    def end(self) -> bool:
        if not self._data.login:
            print("Failed to log out. Session not started yet")
            return False
    ...
    def is_host_up(self) -> bool:
        if gazu.client.host_is_up():
            ...
            print("Error: Failed to reach host at: %s", self.host)
            return False

    def _login(self) -> None:
        session_dict = gazu.log_in(self._email, self._passwd)
        self._data.update(session_dict)
        print("Login was successful. Session started with user %s", self.email)
    ...
    def host(self, host: str) -> None:
        ...
        if host:
            ...
            if not gazu.client.host_is_valid():
                print("Error: Host is not valid: %s", host)
...
class Sequence(Entity):
    ...
    def get_all_scenes(self) -> List[Shot]:
        """UNUSED"""
        return []
...
class Shot(Entity):
    ...
    def get_dir(self, context) -> str:
        project_root_dir = project_root_dir_get(context)
